...
class MrpBom(models.Model):
    ...
    def create(self,vals):
        ...
        if res.routing_id and config.process_costing == 'workcenter':
            for operation in res.routing_id.operation_ids:
                ...
                new.append((1,0,value1))
                res.write({'bom_labour_cost_ids': [(new)]}) 
            ...
            for operation in res.routing_id.operation_ids:
                ...
                new2.append((1,0,value2))
                res.write({'bom_overhead_cost_ids': [(new2)]})  
        return res                  
    ...
    def write(self,vals):
        ...
        for material in self.bom_line_ids:
            product_id = self.env['product.template'].search([('name','=',material.product_id.name)],limit=1)
            vals = {
                    'product_id' : product_id.id,
                    'planned_qty' : material.product_qty,
                    'uom_id' : material.product_uom_id.id,
        ...
        if vals.get('routing_id') and config.process_costing == 'workcenter':
            ...
            for operation in mrp_routing_obj.operation_ids:
                ...
                temp.append((0,0,value3))
                self.write({'bom_labour_cost_ids': [(temp)]}) 
            ...
            for operation in mrp_routing_obj.operation_ids:
                ...
                tamp.append((0,0,value4))
                self.write({'bom_overhead_cost_ids':[(tamp)]}) 
